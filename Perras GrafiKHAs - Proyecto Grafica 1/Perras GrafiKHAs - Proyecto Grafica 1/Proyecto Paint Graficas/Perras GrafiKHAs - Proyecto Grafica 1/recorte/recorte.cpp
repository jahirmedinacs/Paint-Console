        ...
        if(bin[i]!=-1&&bin[i+1]!=-1)
        {
              if((bin[i]|bin[i+1])==0)
            ...
            else if((bin[i]&bin[i+1])!=0)
                {
                    ...
                    glBegin(GL_LINES);
                        glVertex2f(arr[i/2].a,arr[i/2].b);
                        glVertex2f(arr[i/2].c,arr[i/2].d);
                    glEnd();
                }
            else if((bin[i]&bin[i+1])==0)
                {
                    for(int j=i;j<i+2;j++)
                    {
                        while(bin[j]!=0)
                        {
                            ...
                            if(bin[j]&(1))//izquierda
                            {
                                ...
                                if(j&1)
                                    {
                                        double xi=arr[j/2].c,xj=arr[j/2].d;
                                        arr[j/2].d=arr[j/2].d+(((xmin*1.0)-arr[j/2].c)*m),arr[j/2].c=xmin;
                                        glColor3f(0.0,0.0,0.0);
                                        glBegin(GL_LINES);
                                            glVertex2f(arr[j/2].c,arr[j/2].d);
                                            glVertex2f(xi,xj);
                                        glEnd();
                                    }
                                else
                                    {
                                        double xi=arr[j/2].a,xj=arr[j/2].b;
                                        arr[j/2].b=arr[j/2].b+(((xmin*1.0)-arr[j/2].a)*m),arr[j/2].a=xmin;
                                         glColor3f(0.0,0.0,0.0);
                                        glBegin(GL_LINES);
                                            glVertex2f(arr[j/2].a,arr[j/2].b);
                                            glVertex2f(xi,xj);
                                        glEnd();
                                    }
