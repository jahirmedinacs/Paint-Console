#ifndef TEAMPGRADIKHAS_H_INCLUDED
#define TEAMPGRADIKHAS_H_INCLUDED
...
void elipse(pair<double,double> centro, double radio1, double radio2, bool relleno, color colorDataBoundaries,color colorDataFill)
{
...
float Redondeo(float n){return (1.0*round(n*100))/100;}

void regularPolygonPlot(vector < pair < double , double > > puntos, bool relleno, color colorDataBoundaries,color colorDataFill )
{
    double xi, yi, xf, yf, auxX, auxY,yMenor,yMayor;

    set<double>conjuntoY;
    set<double>con;
    ...
    {
        conjuntoY.insert(puntos[i].second);
    }
    yMenor = *(conjuntoY.begin());
    yMayor = *(conjuntoY.rbegin());
    conjuntoY.clear();
    ...
    {
        glBegin(GL_LINES);
            glVertex2f(puntos[i].first, puntos[i].second);
            glVertex2f(puntos[(i+1)%puntos.size()].first, puntos[(i+1)%puntos.size()].second);
        glEnd();
    }
    ...
    if(relleno==true)
    {
        for(int i=0; i<puntos.size(); i++){
            xi = puntos[i].first;
            yi = puntos[i].second;
            xf = puntos[(i+1)%puntos.size()].first;
            yf = puntos[(i+1)%puntos.size()].second;
            if(yf<yi){
                auxX = xi; xi = xf; xf = auxX;
                auxY = yi; yi = yf; yf = auxY;
            }
            double invP = Redondeo((1.0*(xf - xi))/(4*(yf - yi)));
            if(yi != yf)
            {
                int aux;
        ...
        int k=-1;
        double x0, x1;
        for(iter = conjuntoY.begin(); iter!=conjuntoY.end(); iter++)
        {
                k++;
                x0 = *(equis[k].begin());
                x1 = *(equis[k].rbegin());
...
#endif // TEAMPGRADIKHAS_H_INCLUDED
